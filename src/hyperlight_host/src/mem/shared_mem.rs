...
impl ReadonlySharedMemory {
    ...
    /// `FILE_MAP_COPY`, so guest writes through this mapping go to
    /// private copy-on-write pages rather than the backing file —
    /// matching Linux's `MAP_PRIVATE` semantics. WHP/MSHV need the
    /// host-side view to be writable so the hypervisor can handle
    /// CoW faults originating from the guest; a read-only mapping
    /// would fail with an access violation on first write.
    #[cfg(target_os = "windows")]
    fn from_file_windows(file: &std::fs::File, total_size: usize) -> Result<Self> {
        ...
        // written by to_file(), so it's at least offset + len + PAGE_SIZE
        // = total_size bytes.
        let handle =
        ...
        // with FILE_MAP_COPY — reads come from the file; writes allocate
        // private pages transparently on first fault.
