...
impl MultiUseSandbox {
    ...
    pub fn from_snapshot(snapshot: Arc<Snapshot>) -> Result<Self> {
        ...
        // Inherit the snapshot's sandbox id. Without this, a caller who
        // does `from_snapshot(s)` then `restore(s)` immediately hits
        // SnapshotSandboxMismatch — the new sandbox gets a fresh id from
        // from_uninit while the snapshot still carries its originating
        // sandbox's id. Conceptually the snapshot IS this sandbox's
        // identity when loaded from disk.
